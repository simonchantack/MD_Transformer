"""
=============================================================================
  ENCODER FUSION EXPERIMENT — PatchTST Dual-Dim RUL Model  (WHITE THEME)
=============================================================================
  PURPOSE
  -------
  ...
  This is a WHITE-BACKGROUND rendering variant of `encoder_fusion_experiment.py`.
  All numerical / experimental logic is identical; only the matplotlib
  palette has been re-tuned for legibility on a white canvas.
...
# =============================================================================
#  SECTION 6 - VISUALIZATION  (WHITE-BACKGROUND PALETTE)
# =============================================================================

# -- Custom light-theme colour palette -------------------------------------
# Hues chosen so they remain easily distinguishable on a white canvas
# (sufficient saturation + darkness to clear WCAG contrast on #FFFFFF).
TEMPORAL_COLOR = "#0E7C7B"    # deep teal
CHANNEL_COLOR  = "#C0392B"    # deep coral / brick red
FULL_COLOR     = "#B8860B"    # dark goldenrod
BG_COLOR       = "#FFFFFF"    # canvas (figure) background
PANEL_COLOR    = "#FAFAFA"    # subtle off-white for axes facecolor
TEXT_COLOR     = "#1A1A1A"    # near-black for primary text
GRID_COLOR     = "#D0D0D0"    # light gray grid / spines
MUTED_COLOR    = "#6B6B6B"    # mid-gray for de-emphasised text
WARN_COLOR     = "#C2410C"    # darker orange for warnings
EDGE_COLOR     = "#E5E5E5"    # very light gray for bar edges (separation)

_pal = [TEMPORAL_COLOR, CHANNEL_COLOR, FULL_COLOR, "#6D28D9", "#047857"]
...
def make_experiment_figure(
    ...
    save_path: str = "fusion_experiment_results_whitebackground.png",
):
    ...
    bars = ax0.barh(metrics_labels, metrics_vals, color=bar_colors, height=0.45, alpha=0.90)
    ax0.set_xlim(0, 1)
    ...
    ax0.text(0.5, -0.18, interpretation, transform=ax0.transAxes,
             ha="center", color=MUTED_COLOR, fontsize=8, style="italic")
    ...
    ax1.bar(bin_centers, counts, width=(bins[1] - bins[0]) * 0.85,
            color=TEMPORAL_COLOR, alpha=0.80, edgecolor=EDGE_COLOR, linewidth=0.4)
    ...
    bars3 = ax3.bar(ab_labels, ab_rmse, color=ab_colors, width=0.5, alpha=0.90,
                    edgecolor=EDGE_COLOR, linewidth=0.8)
    ax3.set_ylabel("RMSE (down better)", color=TEXT_COLOR)

    best_val = min(ab_rmse)
    for bar, val in zip(bars3, ab_rmse):
        clr = TEXT_COLOR if val == best_val else MUTED_COLOR
    ...
    bars4 = ax4.bar(ab_labels, ab_mae, color=ab_colors, width=0.5, alpha=0.90,
                    edgecolor=EDGE_COLOR, linewidth=0.8)
    ax4.set_ylabel("MAE (down better)", color=TEXT_COLOR)

    best_mae_val = min(ab_mae)
    for bar, val in zip(bars4, ab_mae):
        clr = TEXT_COLOR if val == best_mae_val else MUTED_COLOR
    ...
    # -- Panel 6: Ablation - R^2 bar chart ---------------------------------
    ax5 = fig.add_subplot(gs[1, 2])
    _style_ax(ax5, "Ablation Study  -  R^2")

    ab_r2 = [ablation_results["full"]["r2"],
              ablation_results["temporal_only"]["r2"],
              ablation_results["channel_only"]["r2"]]

    bars5 = ax5.bar(ab_labels, ab_r2, color=ab_colors, width=0.5, alpha=0.90,
                    edgecolor=EDGE_COLOR, linewidth=0.8)
    ax5.set_ylabel("R^2  (up better)", color=TEXT_COLOR)

    best_r2_val = max(ab_r2)
    for bar, val in zip(bars5, ab_r2):
        clr = TEXT_COLOR if val == best_r2_val else MUTED_COLOR
    ...
    sc6 = ax6.scatter(pca_t[idx, 0], pca_t[idx, 1],
                      c=y_true[idx], cmap="viridis", s=12, alpha=0.75, linewidths=0)
    cb6 = plt.colorbar(sc6, ax=ax6)
    cb6.ax.yaxis.set_tick_params(color=TEXT_COLOR)
    cb6.ax.tick_params(labelcolor=TEXT_COLOR)
    cb6.outline.set_edgecolor(GRID_COLOR)
    ...
    sc7 = ax7.scatter(pca_c[idx, 0], pca_c[idx, 1],
                      c=y_true[idx], cmap="viridis", s=12, alpha=0.75, linewidths=0)
    cb7 = plt.colorbar(sc7, ax=ax7)
    cb7.ax.tick_params(labelcolor=TEXT_COLOR)
    cb7.outline.set_edgecolor(GRID_COLOR)
    ...
    if not any(math.isnan(v) for v in mi_results.values()):
        ...
        ax8.bar(mi_labels, mi_vals, color=[TEMPORAL_COLOR, CHANNEL_COLOR],
                width=0.4, alpha=0.90, edgecolor=EDGE_COLOR)
    ...
    bars9 = ax9.bar(grad_labels, grad_vals, color=[TEMPORAL_COLOR, CHANNEL_COLOR],
                    width=0.4, alpha=0.90, edgecolor=EDGE_COLOR)
    ax9.set_ylabel("Mean ||grad||_2", color=TEXT_COLOR)
    ...
    ax10.plot(xs, y_t_only[idx2],   color=TEMPORAL_COLOR,  linewidth=1.0, label="Temporal Only", alpha=0.85, linestyle="--")
    ax10.plot(xs, y_c_only[idx2],   color=CHANNEL_COLOR,   linewidth=1.0, label="Channel Only", alpha=0.85, linestyle=":")
    ...
    lines = [
        ...
        (("Fusion is BENEFICIAL" if fusion_wins_rmse else "Fusion did not win ablation"),
         FULL_COLOR if fusion_wins_rmse else WARN_COLOR, 10, "bold", "normal"),
        (("Encoders capture distinct structure." if cka_value < 0.5 else "Encoders may be redundant."),
         TEXT_COLOR, 9, "normal", "italic"),
    ]
...
def run_fusion_experiment(
    ...
    """
    Full experiment pipeline (white-background rendering).
    ...
    """
    print("=" * 65)
    print("  ENCODER FUSION EXPERIMENT  (white background)")
    print("=" * 65)
    ...
    # Subsample for CKA speed (O(N^2) kernel)
    ...
    for mode in ["full", "temporal_only", "channel_only"]:
        ...
        print(f"      [{mode:>14s}]  RMSE={ablation_results[mode]['rmse']:.4f}  "
              ...
              f"R^2={ablation_results[mode]['r2']:.4f}")
    ...
    print(f"      Temporal grad-norm : {grad_attr['temporal_grad_norm']:.6f}")
    print(f"      Channel  grad-norm : {grad_attr['channel_grad_norm']:.6f}")
...
#       device           = device,
#       save_path        = "fusion_experiment_results_whitebackground.png",
#   )
#
# =============================================================================
