...
# %%
# # # Normalize RUL - target variable
# scaler = StandardScaler()
# y_train_arr = norm_df['rul'].to_numpy()
# y_test_arr = y_test.to_numpy()
# y_train_reshaped = scaler.fit_transform(y_train_arr.reshape(-1, 1))
# y_test_reshaped = scaler.transform(y_test_arr.reshape(-1, 1))

# norm_df['rul'] = pd.Series(y_train_reshaped.reshape(-1))
# y_test = pd.Series(y_test_reshaped.reshape(-1))

# %% 
# Training and testing data preparation
# X = norm_df.drop(columns = ['OperCluster'])
# X_test = norm_df_test.drop(columns = ['OperCluster']) # X_test from blind test set (has no target)
...
# %%
# if window_type == 'sliding':
#     train_dataset = supDataset(data_list=X_train.tolist(), targets=y_train)
#     val_dataset = supDataset(data_list=X_val.tolist(), targets=y_val)
#     test_dataset = supDataset(data_list=X_testf.tolist(), targets=y_test)
...
def fit_patchtst_dualdim_rul(train_dataloader, val_dataloader, test_dataloader,
    ...
    #     stride=cfg.stride,
    #     use_feature_attn=cfg.use_feature_attn,
    #     d_model_c=cfg.d_model_c,
    #     n_heads_c=cfg.n_heads_c,
    #     n_layers_c=cfg.n_layers_c,
    #     d_ff_c=cfg.d_ff_c,
...
#     L = window_size, 
#     # step_size = 2,
#     patch_len_t = 30,  # was chnaged from 30 (RMSE 11.76) to 20 (RMSE( ??))
#     stride_t = 8,
...
X_train, X_val , y_train, y_val  = train_test_split(X_train_sw, y_train_sw, test_size=0.2 , random_state=333)
...
# %%
print(f"Best Random State: {best_randon_state}")
# %%
