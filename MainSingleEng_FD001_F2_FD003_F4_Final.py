...
# File: MainSinglEng_FD001_FD002_FD003_FD004.py
# Code execute entire pipeline of the Multi-Dimension Transformer architecture
# From import, cleaning, and preparing the data to building, training, and evaluating the Multi-Dimension Transformer  
import os
import math
import time
import copy
...
# %%
# Load Training Data base on engine type. Engine typ can be FD001, FD002, FD003, of FD004
...
# %%
# Parameters can be changed based on engine type to get best performance
cfg = TrainConfig(
    ...
    C = len(features),
    L = window_size,     
    patch_len_t = 10,  # was changed from 30 (RMSE 11.76)
    stride_t = 5, #8,
    patch_len_c = 3, #len(features),
    stride_c = 1, #len(features)//2,
...
import random
from sklearn.model_selection import train_test_split
best_rmse = 20  # Set RMSE high as a starting reference

# Use fixed random states or generate a list of 3 random integers between 0 and 200
rand_cnt = 1
# random_state_lst = [random.randint(0, 350) for _ in range(3)] 
random_state_lst = [42, 137, 271] #[341, 333, 250, 42,378, 350] 
...
print(f'Best Total Score: {best_total_score:.2f}')
...
plt.plot(y_test[ind], label='Actual RUL')
plt.plot(y_predb[ind], 'ro-', label='Predicted RUL')
...
total_score = best_scores[0] + best_scores[1]
print(f"Total Score Metric: {total_score:.2f}")
...
# %%
# After evaluation, can reload model to visualize distribution of errors (True - Prediction)
# ======================================
# LOAD BEST SAVED MODEL
# ======================================
import os
model_filename = "dualdimen_patchtst_rul_best_FD001.pt"
...
# model_path = r"//BestModels//dualdimen_patchtst_rul_best_FD001.pt"
# model_path = r"BestModels/DualDimension_PatchTST_Sensor/F001/dualdimen_patchtst_rul_best_rand_selected_FD001_RMSE13-36.pt"

# Recreate the model architecture
C = len(features)
# model_loaded = PatchTST_RUL_Model(C, window_size, cfg).to(device)

model_loaded = PatchTST_RUL_Model(
    C = cfg.C,
    L = cfg.L,
...
test_loss, test_mets, y_test_arr, y_pred_arr = evaluate(
    model_loaded, test_dataloader, device, criterion
)

print(f"[LOADED MODEL] RMSE={test_mets['RMSE']:.4f}  MAE={test_mets['MAE']:.4f}  R2={test_mets['R2']:.4f}")

# %%
# ======================================
# COMPUTE ERRORS
# ======================================
errors = y_pred_arr - y_test_arr  # (N,)
...
# ======================================
plt.figure(figsize=(10,5))
sns.histplot(errors, kde=True, bins=40, color='steelblue')
...
sns.boxplot(x=errors, color='lightcoral')
...
plt.title("Violin Plot of Prediction Error Distribution")
plt.grid(alpha=0.3)
plt.show()
