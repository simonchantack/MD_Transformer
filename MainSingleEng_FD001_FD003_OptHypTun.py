...
# %%
# Identify columns to drop to mitigate multi-colinearity 
col_to_drop = []
if len(high_corr) > 0:
...
def objective(trial):
    ...
    cfg = TrainConfig(
        ...
        patch_len_c = trial.suggest_int("patch_len_c", 5, len(features), step=1), 
        stride_c = trial.suggest_int("stride_c", 1, len(features)//2, step=1), 
    ...
    best_rmse = 20
    # FD001 Random_State: ; FD003 Random_State: 341
    X_train, X_val , y_train, y_val  = train_test_split(X_train_sw, y_train_sw, test_size=0.2 , random_state=341)  
    ...
    print(f"Ready For Training Windowed shapes -> Channels: {C}, Lookback: {L}; Hypertuning")
...
study_name = "patchtst_dualdim_rul_optuna"
...
# # Once tuning finishes, load and reuse the best parameters
# study = joblib.load("optuna_patchtst_dualdim_rul.pkl")
# best_params = study.best_params

# # Override TrainConfig with best found parameters
# cfg = TrainConfig(**{**vars(cfg), **best_params})
...
# %%
print(f'Best random state was: {random_state}')
